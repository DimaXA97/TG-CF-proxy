...
- Entry: SOCKS5
- Relay: Cloudflare WS (transparent, WITH proper MTProto framing)
- Fallback: Direct TCP passthrough
- Features: Subnet DC resolver, Browser/TLS passthrough, CF domain retry
- Version: 2.2.0 (Fixed GIF/media hang, 1.0.7 framing logic adapted)
"""
...
_TELEGRAM_IP_RANGES = [
    ...
    (struct.unpack('!I', _socket.inet_aton('5.28.195.0'))[0], struct.unpack('!I', _socket.inet_aton('5.28.195.255'))[0]),
...
def _is_browser_ws_init(data: bytes) -> bool:
    if len(data) < 16:
        return False
    if data[:3] in (b'GET', b'POS', b'HEA', b'OPT'):
        return True
    try:
...
async def _tcp_last_resort(reader, writer, dst, port, init_data: bytes, label):
...
async def _handle_client(reader, writer, cf_domain: str):
    ...
    try:
        ...
        if not first:
            log.debug("[%s] no data after SOCKS5", label)
            writer.close()
            return
        fb = first[0]
        ...
        await _bridge_cf_ws(reader, writer, ws, label, dc, is_media, splitter, init_data=init)
